...
#--------------------------------------------------------------------
sql_create = '''
create table if not exists score_UnixBench_2thread(
...
cursor.execute('truncate table score_UnixBench_2thread;')
sql_insert = '''
insert into score_UnixBench_2thread(node_num,test_option,ref_data,score) values(%s, %s, %s, %s);
'''

i=1
with io.open('UnixBench_2thread.csv', mode= 'rt', encoding='utf-8') as file_handler:
    # 通过迭代器，遍历csv文件中的所有样本
