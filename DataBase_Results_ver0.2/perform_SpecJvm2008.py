...
#读取csv文件插入表数据
sql_insert = '''
insert into score_SpecJvm2008(Tag,node_num,compiler,compress,crypto,derby,mpegaudio,`scimark.large`,`scimark.small`,serial,startup,sunflow,xml,`Composite-result`) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);

'''
#------------------------------------------------------------------------------
trunc_sql = 'truncate table score_SpecJvm2008'
#------------------------------------------------------------------------------
        ...
        try:
            ...
            with io.open(input_file, mode= 'rt', encoding='utf-8') as file_handler:
                ...
                for line in islice(lines, 1, None):
                   ...
                   #print(res)
                   if res != ['']:
                     self.cur.execute(sql,[res[0],res[1],res[2],res[3],res[4],res[5],res[6],res[7],res[8],res[9],res[10],res[11],res[12],res[13]])
...
if __name__ == '__main__':
    ...
    try:
        ...
        #trunc_sql = 'truncate table score_SpecJvm2008'
        sql_delete = "delete from score_SpecJvm2008 where Tag ='%s'" %(test_Tag)
