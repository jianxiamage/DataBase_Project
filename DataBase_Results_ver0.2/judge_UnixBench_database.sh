...
for i in $num_th;
do
  ls $destPath/ | grep "${TestCase}" |grep "csv" |grep "${i}thread"
  if [ $? -eq 0 ];
  then
    echo "*******************************************************************************"
    echo "Begin to write to database for UnixBench $i thread..."
    echo "*******************************************************************************"
    TestMode="UnixBench_${i}thread"
    sh UnixBench_${i}thread_database.sh $TestType $Platform $TestMode
  fi
done
#----------------------------------------------------------------------------------------
